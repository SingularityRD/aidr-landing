...
import { notifyIncidentCaseEscalations } from "@/lib/control-plane/incident-notifications";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.error({ error: message, uid }, "Incident case notification scheduler failed");
    await recordControlPlaneAudit({
      action: "incident-case-escalation-notify",
      outcome: "failed",
      uid,
    ...
    return json(500, { error: "incident_case_notification_failed" });
  }
}
